...
class Character {
    constructor(x, y, name) {
        ...
        //subroutines
        this.subroutines = ['spin','halfspin'];
...
class Flyer extends PowerBall {
    ...
    flyto(vec){
        if(!this.doflyto && this.flytovec === null){
            ...
            let dist = Math.abs(dir.x)
            if(dist>120){
                //figuere out a function based solely on distace left
                let target = new Vector(dir.x, dir.y - Math.sin((dist/width)*Math.PI*20)*500);
                ...
                this.flytovec = null;
                this.doflyto = false;
                this.landing_emitter.emit('flyto');
            }
