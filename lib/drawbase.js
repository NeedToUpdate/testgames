...
let USE_SET_TIMEOUT_FALLBACK_ON_SLOWDOWN = false;
let GLOBAL_TIME_FOR_FALLBACK = 0;
let ARRAY_OF_TIMES_FOR_FALLBACK = [];
function createFallbackLoopFunction(fn){
    //function provided is the loop() function
    loop = fn;
    GLOBAL_TIME_FOR_FALLBACK = 0;
    function fancyLoop(time){
        let delta_t = time-GLOBAL_TIME_FOR_FALLBACK;
        ARRAY_OF_TIMES_FOR_FALLBACK.push(delta_t);
        if(ARRAY_OF_TIMES_FOR_FALLBACK.length>40) ARRAY_OF_TIMES_FOR_FALLBACK.splice(0,1);
        if(ARRAY_OF_TIMES_FOR_FALLBACK.reduce((a,b)=>a+b) > 680){
            USE_SET_TIMEOUT_FALLBACK_ON_SLOWDOWN = true;
            MAINLOOP = setInterval(loop,17);
            start = function(){
                LOOPING = true;
                MAINLOOP = setInterval(loop,17)
            };
        }
        if(LOOPING && !USE_SET_TIMEOUT_FALLBACK_ON_SLOWDOWN){
            loop();
    ...
    start = function () {
        LOOPING = true
        GLOBAL_TIME_FOR_FALLBACK = window.performance.now();
