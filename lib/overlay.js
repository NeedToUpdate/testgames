            ...
            arr.forEach((x, j) => {
                ...
                if (i !== this.num_of_layers - 1) {
                    for (let k = 0; k < this.layers[i + 1].length; k++) {
                        let rowRes2 =
                            this.height / (this.layers[i + 1].length + 1);
                        ...
                        let r = new Div(
                            ...
                            this.y + rowRes * (j + 1),
                            this.getCol(
                                this.nn.layers[i].weight_array[0][0].values[k][
                                    j
                                    ]
                            ),
                            w,
    ...
    draw() {
        this.circles.forEach((arr, i) => {
            arr.forEach((c, j) => {
                ...
                if (i === 0) {
                    val =
                        (this.nn.layers[i].input_values.values[j][0] || 0) *
                        255;
                } else {
                    val =
                        this.nn.layers[i - 1].output_values.values[j][0] * 255;
                }

                c.shape.style.background =
                    "rgb(" + val + "," + val + "," + val + ")";
            });
        });
