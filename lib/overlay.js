class nnOverlay{
    ...
    init(){
        ...
        this.layers.forEach((arr,i)=>{
            if(i!== this.num_of_layers-1 && this.nn.layers[i].isSpecial()){
                return;
            }
            ...
            this.lines[i][j] = [];
                ...
                if(i!==this.num_of_layers-1){
                     for(let k= 0; k<this.layers[i+1].length; k++){
                         ...
                         let r = new Div(this.x+colRes*(i)+10,this.y + rowRes*(j+1), this.getCol(this.nn.layers[i].weights.values[k][j]), w,0,true, ang*(180/Math.PI),true);
