...
class Character extends Blank{
    ...
    powerUp(num,power) {
        ...
        if (!this.isPoweringUp) {
            ...
            let attack_sprite = new Img(getImage(atkname).cloneNode(), (attack.x < 0 ? 0 : attack.x), attack.y, 50).fromCenter().onLoad(()=>{
                ...
                attack.sprite.set('will-change','top,width,left');
                if (this.isFacingRight) {
                    attack.faceRight();
                } else {
                    attack.faceLeft();
    ...
    shoot(power) {
        ...
        if (this.isPoweringUp && this.attachments[power]) {
            ...
            atk.stopHover();
            atk.forces.push(new Vector(this.isFacingRight?atk.width/2:-atk.width/2,0));
            this.isPoweringUp = false;
            return atk;
        }
    }
    ...
    constructor(x,y,name){
       ...
       doFlyTo(target, max_v){
           if (!this.isDoingFlyTo) {
               ...
               if (dist > 120) {
                   let target = new Vector(dir.x, dir.y - Math.cos((distX / width) * Math.PI * 20) * 500);
                   let steer = target.copy().sub(this.v).set(config.max_v);
                   this.addForce(steer);
               } else if (dist< 119 && dist > 5) {
