class NeuralNetwork {
    constructor(layer) {
        ...
        if (arguments.length >= 2) {
            ...
            this.num_of_outputs = this.layers[
    ...
    initdefault() {
        ...
        this.layers[
        this.num_of_layers - 1
            ].config.activation_fn = this.config.output_activation_fn;
    ...
    addLayer(output_nodes, type, config, is_output) {
        ...
        if (arguments.length < 2)
            console.log(
                `%cdon't forget to set config of this layer!`,
                "color:teal"
            );
    ...
    removeLayer(index) {
        ...
        this.layers[index - 1].num_of_outputs = this.layers[
            index
            ].num_of_inputs;
        this.init();
    }
    ...
    randomizeAll() {
        if(this.initialized) {
            ...
            console.error('Unable to .randomizeAll() on an uninitialized net')
        }
    }
    ...
    predict(input_array) {
        ...
        if (input_array.length !== this.num_of_inputs) {
            console.error(
                `input (${input_array.length}) must match input nodes (${
                    this.num_of_inputs
                    })`
            );
            return undefined;
    ...
    train(input_array, targets, callback) {
        ...
        if (targets.length !== this.num_of_outputs) {
            console.error(
                `target must match output nodes (${this.num_of_outputs})`
            );
            return undefined;
        }
        if (this.config.learning_fn === "SGD") {
            if (this.config.is_alpha_calculated) {
                this.setLearningRate(
                    this.config.learning_alpha * this.calc_loss(),
                    true
                );
            } else {
                //first hundred or so steps will have insane loss numbers
                if (this.training_times > this.config.skip_alpha_steps) {
                    this.config.learning_alpha =
                        this.config.learning_rate / this.calc_loss();
        ...
        if (this.sequence_training) {
            ...
            error_array = this.sequence.outputs.map((matrix, index) => {
                if (this.config.loss_fn !== "default") {
                    this.error_vals.push(
                        loss_fn[this.config.loss_fn](
                            matrix.flatten(),
                            this.sequence.targets[index].flatten()
                        )
                    );
                }
            ...
            Object.keys(this.sequence).forEach(
                key => (this.sequence[key] = [])
            );
        }
        let prev_error;
        if (!this.sequence_training) {
            if (this.config.loss_fn !== "default") {
                let loss = loss_fn[this.config.loss_fn](
                    outputM.flatten(),
                    targetM.flatten()
                );
        ...
        this.layers.forEach((x, i) => {
            ...
            if (this.sequence_training) {
                if (this.layers[i].config.type !== "lstm") {
                    //lazy reverse
                    prev_error.reverse();
                    prev_error = prev_error.map(x =>
                        this.layers[i].backPropagate(x)
                    );
    ...
    setLearningRate(num, dontaffectalpha) {
        if (this.config.is_alpha_calculated && !dontaffectalpha) {
            this.config.learning_alpha =
                this.config.learning_alpha * (num / this.config.learning_rate);
        }
    ...
    log() {
        ...
        console.log(
            `%cinputs:  ${this.num_of_inputs}  | outputs:  ${
                this.output_layer.num_of_outputs
                }`,
            "color:darkblue"
        );
    ...
    mix(nnB, rate) {
        ...
        nnA.layers.forEach((layer, k) => {
            layer.weight_array.forEach((group, l) => {
                group.forEach((W, m) => {
                    W.map((x, i, j) =>
                        pick(x, nnB.layers[k].weight_array[l][m].values[i][j])
                    );
                });
            });
            layer.bias_array.forEach((group, l) => {
                group.forEach((B, m) => {
                    B.map((x, i, j) =>
                        pick(x, nnB.layers[k].bias_array[l][m].values[i][j])
                    );
                });
            });
        });
    ...
    static fromJSON(json) {
        ...
        temp.layers.forEach(layer => {
            brain.addLayer(
                layer.num_of_outputs,
                layer.config.type,
                { activation_fn: layer.config.activation_fn },
...
loss_fn = {
    cross_entropy: (output, target) => {
        ...
        output.forEach((out, i) => {
            err +=
                target[i] * Math.log(out) + (1 - target[i]) * Math.log(1 - out);
        });
        return -err;
    },
...
class Layer {
    ...
    init() {
        ...
        if (
            this.config.activation_fn === "default" &&
            !/^conv|lstm|maxpool$/.test(this.config.type)
        ) {
            console.log(
                `%cno activation function selected for layer, using default: (${
                    this.config.default_activation_fn
                    })`,
                "color:orange"
            );
        ...
        switch (this.config.type) {
            case "lstm":
                ...
                this.weight_array.push([
                    new Matrix(
                        this.num_of_outputs,
                        this.num_of_inputs + this.config.num_of_rns
                    ), //forget gate
                    new Matrix(
                        this.num_of_outputs,
                        this.num_of_inputs + this.config.num_of_rns
                ...
                caches.forEach(cache => {
                    while (cache.length < this.config.time_steps + 1) {
                        cache.push(
                            new Matrix(Array(this.config.num_of_rns).fill(0))
                        );
                    }
                });
                while (
                    this.input_value_cache.length <
                    this.config.time_steps + 1
                    ) {
                    this.input_value_cache.push(
                        new Matrix(
                            Array(
                                this.config.num_of_rns + this.num_of_inputs
                            ).fill(0)
                        )
                    );
                }
            ...
            case "dense":
                this.weight_array.push([
                    new Matrix(this.num_of_outputs, this.num_of_inputs),
                ]);
            ...
            case "maxpool":
                ...
                break;
            case "conv":
                this.config.feature_maps =
                    this.num_of_outputs / this.num_of_inputs;
            ...
            default:
                console.error(
                    "using the default case, maybe a typo for layer type"
                );
        }
        ...
        if (/^random|default/.test(this.config.weights_start)) {
            ...
            let num = Math.sqrt(
                6 /
                ((this.num_of_inputs + this.num_of_outputs) *
                    this.config.kernel_size ** 2)
            );
            this.weight_array.forEach(group => {
                group.forEach(W => {
    ...
    feedForward(input_array) {
        if (!this.initialized) {
            console.error(
                this.config.type + " layer isnt initialized, run .init()"
            );
        }
        if (input_array.length !== this.num_of_inputs) {
        ...
        switch (this.config.type) {
            case "lstm":
                while (this.hidden_cache.length < this.config.time_steps + 1) {
                    this.hidden_cache.push(
                        Array(this.config.num_of_rns).fill(0)
                    );
                }
                //input array dropout
                ...
                input_array = input_array.concat(
                    this.hidden_cache[this.config.time_steps]
                );
                if (this.training) {
            ...
            case "empty":
                ...
                if (this.config.input_dropout_rate > 0 && this.training) {
                    ...
                    output = this.activate(
                        input_array,
                        drop(this.config.input_dropout_rate)
                    );
                }
                ...
                if (this.config.output_dropout_rate > 0 && this.training) {
                    ...
                    output = this.activate(
                        output,
                        drop(this.config.output_dropout_rate)
                    );
                }
                return output;
            case "dense":
        ...
        if (
            (this.config.output_dropout_rate > 0 ||
                this.config.input_dropout_rate > 0) &&
            !this.training
        ) {
            weights = this.weight_array[0][0]
                .copy()
                .mult(
                    Math.max(
                        this.config.output_dropout_rate,
                        this.config.input_dropout_rate
                    )
                );
        }
        ...
        this.output_values = this.outputs_calcd
            .copy()
            .map(activations[this.config.activation_fn]);
        ...
        switch (this.config.type) {
            case "recurrent":
                ...
                this.hidden_cache.push(
                    final_output.slice(rn_offset, this.config.num_of_rns)
                );
    ...
    feedForwardCNN(input_array) {
        ...
        if (this.config.input_dropout_rate > 0) {
            input_array.forEach(arr => {
                arr.forEach(mat => {
                    mat.map(x =>
                        Math.random() < this.config.input_dopout_rate ? 0 : x
                    );
                });
            });
        }
        ...
        for (
            let input_index = 0;
            input_index < input_array.length;
            input_index++
        ) {
            for (let i = 0; i < outputmult; i++) {
                ...
                Array.from(this.config.color_depth).forEach((label, j) => {
                    ...
                    if (this.config.type === "maxpool") {
                        m = Matrix.convolute(
                            input,
                            this.weight_array[i][j],
                            new SomethingElse("maxpool")
                        );
                    } else {
        ...
        if (this.training) {
            this.output_value_cache.push(
                output.map(arr => arr.map(mat => mat.copy()))
            );
        ...
        if (this.config.output_dropout_rate > 0 && this.training) {
            output.forEach(arr => {
                arr.forEach(mat => {
                    mat.map(x =>
                        Math.random() < this.config.output_dropout_rate ? 0 : x
                    );
                });
            });
        }
    ...
    feedForwardLSTM(input_array) {
        ...
        if (
            ...
            weights = this.weight_array[0].map(mat =>
                mat
                    .copy()
                    .mult(
                        Math.max(
        ...
        let forget = Matrix.multiply(weights[0], combined).add(
            this.bias_array[0][0]
        );
        if (this.training) {
        ...
        let inputS = Matrix.multiply(weights[1], combined).add(
            this.bias_array[0][1]
        );
        if (this.training) {
        ...
        let inputT = Matrix.multiply(weights[2], combined).add(
            this.bias_array[0][2]
        );
        if (this.training) {
        ...
        let o_output = Matrix.multiply(weights[3], combined).add(
            this.bias_array[0][3]
        );
        ...
        if (Object.keys(this.cellstate_raw_inputs).length < 1) {
            this.cellstate_raw_inputs = new Matrix(
                Array(this.config.num_of_rns).fill(0)
            );
        }
        ...
        if (this.training) {
            ...
            if (
                this.cellstate_output_cache.length >
                this.config.time_steps + 1
            ) {
    ...
    backPropagate(error_array) {
        ...
        if (
            this.config.norm_gradient_val &&
            gradients.absmax() > this.config.norm_gradient_val
        ) {
            gradients.norm(
                -this.config.norm_gradient_val,
                this.config.norm_gradient_val
        ...
        let weights_deltas = Matrix.multiply(
            gradients,
            this.input_values.transpose(true)
        );
        ...
        if (
            this.config.norm_weight_val &&
            this.weight_array[0][0].absmax() > this.config.norm_weight_val
        ) {
            console.log("normalizing weights");
            this.weight_array[0][0].norm(
                -this.config.norm_weight_val,
                this.config.norm_weight_val
            );
        }
    ...
    backpropagateCNN(input_errors) {
        ...
        if (this.config.flatten_layer) {
            ...
            for (let i = 0; i < this.num_of_inputs; i++) {
                let newmat = Matrix.fromArray(
                    input_errors.splice(0, out_w * out_h),
                    out_h,
                    out_w
                );
            ...
            incoming_errors.forEach((mat, i) => {
                ...
                for (let j = 0; j < out_h; j++) {
                    for (let k = 0; k < out_w; k++) {
                        let index = j * out_h + k;
                        newmat.values[output.maxpoolhack[index][0]][
                            output.maxpoolhack[index][1]
                            ] = mat.values[j][k];
                    }
                }
                if (this.config.flatten_layer) {
                    ...
                    newmat.mult(
                        this.input_value_cache[0][i][0]
                            .copy()
                            .map(d_activations[this.config.activation_fn])
                    );
                }
        ...
        gradients.forEach((grad, i) => {
            ...
            if (
                this.config.norm_gradient_val &&
                grad.absmax() > this.config.norm_gradient_val
            ) {
                grad.norm(
                    -this.config.norm_gradient_val,
                    this.config.norm_gradient_val
            ...
            Array.from(this.config.color_depth).forEach((color, k) => {
                this.weight_array[i][k].sub(
                    grad.mult(this.config.learning_rate)
                );
                //weight clipping
                if (
                    this.config.norm_weight_val &&
                    this.weight_array[i][k].absmax() >
                    this.config.norm_weight_val
                ) {
                    this.weight_array[i][k].norm(
                        -this.config.norm_weight_val,
                        this.config.norm_weight_val
                    );
                }
    ...
    backPropagateThroughTime(error_array) {
        ...
        for (let i = this.input_value_cache.length; i > 0; i--) {
            ...
            deltaWTT.push(
                Matrix.multiply(
                    gradients,
                    this.input_value_cache[i - 1].transpose(true)
                )
            );
        ...
        if (
            this.config.norm_gradient_val &&
            deltaW.absmax() > this.config.norm_gradient_val
        ) {
            deltaW.norm(
                -this.config.norm_gradient_val,
                this.config.norm_gradient_val
        ...
        if (
            this.config.norm_weight_val &&
            weight_array[0][0].absmax() > this.config.norm_weight_val
        ) {
            weight_array[0][0].norm(
                -this.config.norm_weight_val,
                this.config.norm_weight_val
            );
        }
    ...
    backPropagateThroughTimeLSTM(error_array) {
        ...
        let final_outp_b = [];
        for (n; n > 0; n--) {
            let errors = new Matrix(
                this.sequence_training ? error_array[n - 1] : next_error
            );
            let new_hidden_errors = (
                this.hidden_error_cache[n + 1] ||
                new Matrix(Array(this.num_of_outputs).fill(0))
            ).copy();
            ...
            let c = new_hidden_errors
                ...
                .multiply(
                    this.outputs_calcd_cache[n]
                        .copy()
                        .map(activations["sigmoid"])
                )
                .multiply(
                    this.cellstate_output_cache[n]
                        .copy()
                        .map(d_activations["tanh"])
                )
                .add(
                    (
                        this.cellstate_error_cache[this.config.time_steps] ||
                        new Matrix(Array(this.config.num_of_rns).fill(0))
                    ).multiply(
                        (
                            this.forget_output_cache[n + 1] ||
                            new Matrix(Array(this.config.num_of_rns).fill(0))
                        ).map(activations["sigmoid"])
                    )
                );
            ...
            if (
                this.cellstate_error_cache.length >
                this.config.time_steps + 1
            ) {
            ...
            let error_tanh_i = c
                .copy()
                .multiply(
                    this.i_output_cache[n].copy().map(activations["sigmoid"])
                )
                .multiply(
                    this.a_output_cache[n].copy().map(d_activations["tanh"])
                );
            let error_sigm_i = c
                .copy()
                .multiply(
                    this.a_output_cache[n].copy().map(activations["tanh"])
                )
                .multiply(
                    this.i_output_cache[n].copy().map(d_activations["sigmoid"])
                );
            let error_forget = c
                .copy()
                .multiply(
                    this.cellstate_output_cache[n - 1] ||
                    new Matrix(Array(this.num_of_outputs).fill(0))
                )
                .multiply(
                    this.forget_output_cache[n]
                        .copy()
                        .map(d_activations["sigmoid"])
                );
            ...
            let next_forg = Matrix.multiply(
                this.weight_array[0][0].transpose(true),
                error_forget
            );
            let next_sigm = Matrix.multiply(
                this.weight_array[0][1].transpose(true),
                error_sigm_i
            );
            let next_tanh = Matrix.multiply(
                this.weight_array[0][2].transpose(true),
                error_tanh_i
            );
            let next_outp = Matrix.multiply(
                this.weight_array[0][3].transpose(true),
                error_output
            );
            ...
            let next_hidden_error = temp_error.splice(
                this.num_of_inputs +
                this.config.num_of_rns -
                this.num_of_outputs,
                this.num_of_outputs
        ...
        let gradient_sums = [
            final_forg
                .reduce((tot, x) => tot.add(x))
                .mult(this.config.learning_rate),
            final_sigm
                .reduce((tot, x) => tot.add(x))
                .mult(this.config.learning_rate),
            final_tanh
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (
                this.config.norm_weight_val &&
                this.weight_array[0][i].absmax() > this.config.norm_weight_val
            ) {
                this.weight_array[0][i].norm(
                    -this.config.norm_weight_val,
                    this.config.norm_weight_val
                );
            }
    ...
    log() {
        console.groupCollapsed(
            `%c == ${this.config.type} layer ==`.padStart(
                24 + (this.config.type.length - 4) / 2,
                " "
            ),
            "color:purple"
        );
