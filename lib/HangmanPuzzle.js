class Puzzle extends Rectangle {
  ...
  createLines() {
    let x = this.width * 0.01;
    let maxwidth = this.width * 0.98;
    let y = this.height * 0.8;
    let linewidth = (maxwidth * 0.9) / this.word.length;
    if (linewidth > width * 0.06) linewidth = width * 0.06;
  ...
  createTitle() {
    let title = new P(this.INTERNAL_TITLE_STRING, this.width / 2, this.height * 0.1).fromCenter();
    title.size = r(width / 29) + "px";
    title.width = this.width * 0.98;
  ...
  styleUp(team) {
    if (team === "A") {
      ...
      this.INTERNAL_TITLE_DIV.set("fontFamily", "mk1");
    } else {
  ...
  createInputBox(x, y, w, h) {
    ...
    let button = new Rectangle(box.width * 0.1, inputBoxWidth * 1.8 + 20, box.width * 0.8 - 10, box.height * 0.1).asOutline("green", r(width / 319));
