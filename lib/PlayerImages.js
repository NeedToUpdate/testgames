...
async function choose_your_fighter_2Team(isTeamA) {
  return new Promise((resolve) => {
    ...
    chosen_nums.forEach((num, i) => {
      ...
      let actual_name = convertToReadableName(ALL_CHARACTER_CONFIG[num].name);
      ...
      let widthOfCharScreen = screenw / chosen_nums.length;
      if (name.width + 20 > widthOfCharScreen) {
        let widthOfEachLetter = name.width / name.string.length;
        console.log(actual_name, r(widthOfCharScreen / widthOfEachLetter) - 3);
        name.string = actual_name.slice(0, r(widthOfCharScreen / widthOfEachLetter) - 3) + "..";
      }
...
function setUpCharacter_2Team(isTeamA, num) {
  return new Promise((resolve) => {
    ...
    let sprite = new Img(IMAGE_PATH + IMAGE_CONFIG.characters.path + vals.name + ".png", 0, 0, width / 8).fromCenter().onLoad(() => {
