...
let characters = [
    'cat', 'spiderman', 'ultraman', 'ironman',
    'peppa_pig', 'spongebob', 'monkey', 'tiger', 
    'shark', 'cinderella', 'elsa', 'ariel', 
    'dead_fish', 'mr_krabs', 'thanos', 'pikachu', 
    'dr_strange', 'superman', 'genie', 'simba', 
    'captain_marvel', 'mewtwo', 'sailor_moon', 'venom', 
    'thor', 'ultraman2', 'ultraman3', 'olaf', 
    'hulk', 'gundam', 'optimus', 'snow_white', 
    'deadpool', 'goku', 'ash', 'charizard', 
    'captain_america', 'nakoruru', 'mario', 'emmet', 
    'wyldstyle', 'tree', 'link', 'isabelle', 
    'doraemon', 'hello_kitty', 'penguin','rainbow',
    'zebra','sofia', 'baby', 'starfish', 
    'bananaman' , 'arlo' , 'black_widow' , 'shrek', 
    'batman', 'hawkeye', 'hermione','harry_potter',
    'sleeping_beauty','rapunzel','vision', 'squirtle',
    'anna','monkey_king','shoe','arceus',
    'luffy','ant','dog','dolphin',
    'whale', 'zoro'

];
let powers = [
    'blueenergy', 'web', 'bluebeam', 'fire',
    'pinkenergy', 'krabbypatty', 'banana', 'fire', 
    'water', 'blueenergy', 'ice', 'water', 
    'nuke', 'spongebobdollar', 'blackenergy', 'yellowelectric', 
    'magic', 'fire', 'magic', 'magic', 
    'electric', 'magic', 'pinkenergy', 'blackenergy', 
    'electric', 'magic', 'electric', 'ice', 
    'rock', 'bluebeam', 'bullet', 'bluebeam', 
    'bullet',  'bluebeam', 'fire', 'fire', 
    'fire', 'fire', 'fire', 'legored', 
    'blackenergy',  'apple', 'blueenergy', 'water', 
    'blueenergy', 'pinkenergy', 'fish', 'pinkenergy',
    'fire','robin', 'water', 'stars' , 
    'banana', 'rock' , 'grenade', 'onion', 
     'blackenergy', 'arrow', 'greenenergy','whitemagic',
    'pinkenergy','hairbrush','yellowmagic','water',
    'torch','ki','smellysocks','whitemagic',
    'fire','leaf','bone','water',
    'water', 'katana'
...
async function choose_your_fighter_2Team(isTeamA) {
    return new Promise(resolve => {
        ...
        chosen_nums.forEach((num, i) => {
            ...
            let actual_name = characters[num].split('_').map(x=>x[0].toUpperCase() + x.slice(1,x.length)).join(' ');
            if(!isNaN(actual_name[actual_name.length-1])){
                actual_name = actual_name.slice(0,actual_name.length-1)
            }
            let name = new P(actual_name, screenx + (screenw / chosen_nums.length) * (i + .1) + 10, 5 + screeny + screenh / 1.5);
