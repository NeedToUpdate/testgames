class Quad {
    ...
    get length(){
        return this.getAllPoints().length;
    }

    removeItem(p,data) {
        if (this.divided) {
            this.inner[this.whichQuad(p)].removeItem(p,data);
        } else {
            let object = this.points.filter(x=>x.data == data)[0];
            if(object) {
                this.points.splice(this.points.indexOf(object),1);
            }else{
                console.log('unable to find this item', p, data)
            }
        }
        return p;
    }
