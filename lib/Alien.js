...
class Alien extends Flyer {
    ...
    doPredict() {
        if(this.isDoingPredict){
            ...
            if(checkObj(letter)){
                ...
                this.MAX_V = mapNum(last_move.mag,0,dist,this._DEFAULT_MAX_V/2,this._DEFAULT_MAX_V) //trial and error's numbers. maybe better way to figure this out
                this.MAX_F = mapNum(last_move.mag,0,dist,this._DEFAULT_MAX_F/2,this._DEFAULT_MAX_F)
                let targetV = letter.p.copy().sub(last_move.mult(this.targetLetter.width).limit(width/6))
                this.pathTo(targetV);
            }
        }else{
            return new Promise(resolve=>{
    ...
    doTaunt() {
        if(this.isDoingTaunt){
            ...
            if(checkObj(letter)){
                ...
                let targetV = letter.p.copy().add(last_move.set(this.targetLetter.width*3.5-last_move.mag))
                this.MAX_V = mapNum(last_move.mag,0,dist,this._DEFAULT_MAX_V/2,this._DEFAULT_MAX_V*1.05) //trial and error's numbers. maybe better way to figure this out
                this.MAX_F = mapNum(last_move.mag,0,dist,this._DEFAULT_MAX_F/2,this._DEFAULT_MAX_F*1.05)
                this.pathTo(targetV);
            }
        }else{
            return new Promise(resolve=>{
                this.cache.doTaunt = {}
                let val = this.targetLetter.width*2
                this.cache.doTaunt.targetVector = new Vector(getRandom(-val,val), getRandom(val/5,val/2));
    ...
    doScare() {
        if(this.isDoingScare){
            let letter = this.targetLetter;
            if(letter.p.copy().sub(this.p).mag > width/20 && !this.cache.doScare.running){
    ...
    doPatrol() {
        function randomPosition(pos) {
            let step_size = getRandom(3) * width/19.22 + width/5
            let poss = [0, 1, 2, 3];
            if (pos.x < step_size) {
