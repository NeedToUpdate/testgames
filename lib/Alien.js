...
class Alien extends Flyer {
    ...
    doPredict() {
        if(this.isDoingPredict){
            ...
            if(checkObj(letter)){
                let last_move = checkObj(letter.last_move)? letter.last_move.copy().add(this.cache.doPredict.targetVector.zeroY) : this.cache.doPredict.targetVector;
            ...
            return new Promise(resolve=>{
                ...
                this.cache.doPredict.targetVector = new Vector(getRandom(-5,5),getRandom(3,7))         
                resolve();
            })
        }
    }
