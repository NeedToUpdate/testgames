    ...
    doPredict() {
        if(this.isDoingPredict){
            ...
            if(checkObj(letter)){
                ...
                this.MAX_V = mapNum(last_move.mag,0,r(width/38.44),this._DEFAULT_MAX_V/2,this._DEFAULT_MAX_V) //trial and error's numbers. maybe better way to figure this out
                this.MAX_F = mapNum(last_move.mag,0,r(width/38.44),this._DEFAULT_MAX_F/2,this._DEFAULT_MAX_F)
                let targetV = letter.p.copy().sub(last_move.mult(r(width/19.22)).limit(width/6))
                this.pathTo(targetV);
            }
        }else{
            return new Promise(resolve=>{
    ...
    doTaunt() {
        if(this.isDoingTaunt){
            ...
            if(checkObj(letter)){
                ...
                let targetV = letter.p.copy().add(last_move.set(width/6-last_move.mag))
                this.MAX_V = mapNum(last_move.mag,0,25,this._DEFAULT_MAX_V/2,this._DEFAULT_MAX_V*1.05) //trial and error's numbers. maybe better way to figure this out
                this.MAX_F = mapNum(last_move.mag,0,25,this._DEFAULT_MAX_F/2,this._DEFAULT_MAX_F*1.05)
                this.pathTo(targetV);
            }
        }else{
            return new Promise(resolve=>{
                this.cache.doTaunt = {}
                let val = r(width/38.44)
