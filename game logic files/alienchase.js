...
function setupAliens() {
    ...
    challenges = new Rectangle(width*0.8,height*0.2, width*.19, height*.6).asOutline('white',3);
    let challengeText = new P('Challenges',0,0).withNoCss()
    challenges.attach(challengeText)
    challengeText.color = 'red'
    challengeText.size = r(width/30) + 'px'
    challenges.set('textAlign','center')
    challengeText.set('margin',0)
    challenges.set('display','none')
    valid_challenge_names.forEach(name=>{
        let p = new P(name).withNoCss();
        challenges.attach(p)
        p.color = 'yellow'
        p.set('margin',0)
        p.set('textShadow', 'black 0 0 2px')
        p.size = r(width/40) + 'px'
        p.shape.addEventListener('click',()=>{
...
function addAlien(color, variety) {
    ...
    return new Promise((resolve, reject) => {
        ...
        previewAliens(selected_aliens, variety).then(() => {
            ...
            config = alien_config[color];
            total_difficulty += config.difficulty;
            difficultyCounter.value = total_difficulty
...
function removeAllAliens(){
    selected_aliens = [];
    previewAliens(selected_aliens, 0)
}
let preview_divs = [];
...
function previewAliens(arrayOfAliens, variety) {
    ...
    let promises = [];
    if(arrayOfAliens.length){
        let clear = new P('clear',LEFT + SPRITE_WIDTH*PER_ROW + PADDING*PER_ROW, TOP*2)
        preview_divs.push(clear)
        clear.set('opacity','0.7')
        clear.shape.addEventListener('click',removeAllAliens)
    }else{
        total_difficulty = 0;
        difficultyCounter.value = total_difficulty;
        difficultyCounter.setBar('backgroundColor', 'limegreen')
...
function checkLetter() {
    ...
    if(previouslyTouchedLetter.isResetting) return;
...
function gameloop(time) {
    ...
    if (THINGS_ARE_DRAGGABLE && ((currentTime|0) % 4 == 0)) {
        checkLetter()
    }
    if (LOOPING) requestAnimationFrame(gameloop)
}

let valid_challenge_names = ['grass', 'ocean','fire' , 'hellokitty', 'beautiful', 'black hole','every color' , 'oh my god']
...
function selectChallenge(name) {
    console.assert(valid_challenge_names.includes(name), name + ' is not a valid challenge.')
    let aliens = getChallenge(name);
    let variety = getRandom(3) //might need changing later
    aliens.forEach(alien => {
...
function tester() {
    ...
    MAINTESTER.setButton(2).name('challenges');
