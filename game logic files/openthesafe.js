...
function ind2xy(index) {
  let stack = 8;
  let top = paper.y + paper.height * 0.1;
  let left = paper.x + paper.width * 0.1;
  let y = index % stack;
  let x = (index / stack) | 0;
  let longest = 0;
  for (let i = x; i > 0; i--) {
    //get the longest word in the previous coloums and get the offsetLeft
...
function setupkeypad() {
  ...
  keypad.set("backgroundImage", "linear-gradient(to bottom right, " + "#eee 0%, #aaa 10%, #ddd 14%, #fff 30%, #999 44%, #ddd 55%, #999 60%, #eee 85%, #ccc 100%" + ")");
  ...
  lettersM.map((x, i, j) => {
    ...
    p.sprite.set("backgroundImage", "linear-gradient(to bottom right, " + "#eee 0%, #e1e1e1 10%, #ddd 14%, #fff 30%, #ddd 60%, #eee 85%, #ccc 100%" + ")");
...
function introMovie() {
  let bank = new Img(IMAGE_PATH + "/bank.png", width * 0.1, height / 7, width / 1.922);
  let burglars = [];
  for (let i = 0; i < 5; i++) {
    let burglar = new Character(width * 0.8 + (width / 35 + ((Math.random() * 10) | 0)) * i, height - height / 8, "burglar");
  ...
  function movieloop() {
    ...
    if (now - time > 10000 && !movieending) {
      ...
      movieending = true;
      hole = new Img(LOADED_IMAGES.brokenhole.cloneNode(), width * 0.5, height * 0.8, width / 8).fromCenter();
      setTimeout(() => {
        playending = true;
      }, 2000);
    }
