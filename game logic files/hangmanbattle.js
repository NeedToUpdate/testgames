...
function setupBackground() {
  return new Promise((resolve) => {
    MAINARENA = new Rectangle(width * 0.2, height * 0.2, width * 0.6 - 5, height * 0.8).asOutline("black", 5);
...
function setup() {
  LOADED_IMAGES = new ImageLoader(
    IMAGE_PATH + "projectiles/",
    ["fire", "dynamite", "shuriken", "knife", "grenade"].map((x) => x + "_projectile")
  );
...
function setUpWord(team, word) {
  if (team) {
    if (Object.keys(teamA.puzzleDiv).length === 0) {
      teamA.puzzleDiv = new Puzzle(word, 0, 0, width * 0.3, height * 0.3, "A");
      teamA.puzzleDiv.set("zIndex", "10");
    } else {
    ...
    if (Object.keys(teamB.puzzleDiv).length === 0) {
      teamB.puzzleDiv = new Puzzle(word, width * 0.7 - 6, 0, width * 0.3, height * 0.3, "B");
  ...
  return new Promise((resolve) => {
    ...
    function setWordsInCircle(wordA, wordB) {
      ...
      for (let i = 0; i < wordArray.length; i++) {
        ...
        let l = Line.fromAngle(
          left + r(width / 310) + Math.cos(((theta * i - 90) / 180) * Math.PI) * r(width / 10),
          top + r(width / 310) + Math.sin(((theta * i - 90) / 180) * Math.PI) * r(width / 10),
          width / 23.33,
          theta * i - 90,
          r(width / 310)
        );
...
function goGiantAndStomp(isPlayerA, val) {
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      team.puzzleDiv.x -= width * 0.15 * (isPlayerA ? 1 : -1);
      team.puzzleDiv.inputBox.x -= width * 0.15 * (isPlayerA ? 1 : -1);
    }, 3000);
    setTimeout(() => {
      ...
      let unsub = fighter.landing_emitter.subscribe("land", () => {
        ...
        setTimeout(() => {
          ...
          fighter.width /= 5;
          fighter.minbounds.y = height * 0.2;
          fighter.minbounds.x = width * 0.21;
          fighter.maxbounds.x = width * 0.79;
          setTimeout(() => {
...
function throwMeteor(isPlayerA, val) {
  ...
  function breakTop(onLeft) {
    ...
    team.puzzleDiv.y -= height * 0.15;
    team.puzzleDiv.x -= height * 0.15 * (isPlayerA ? 1 : -1);
    return {
      clear: function () {
        team.puzzleDiv.removeClass("fastsmoothed");
        l.remove();
...
function resetSome() {
  ...
  teamA.puzzleDiv.x = 0;
  teamA.puzzleDiv.y = 0;
  teamB.puzzleDiv.x = width * 0.7 - width / 160;
  teamB.puzzleDiv.y = 0;
  ...
  playerA.x = width * 0.28;
  playerA.y = height * 0.75;
  playerB.x = width * 0.72;
...
function resetAll() {
  ...
  playerA.height = "";
  playerB.height = "";
  playerA.maxbounds.x = width * 0.79;
  playerA.minbounds.x = width * 0.21;
  playerA.maxbounds.y = height - 20;
  playerA.minbounds.y = height * 0.2;
  playerB.maxbounds.x = width * 0.79;
  playerB.minbounds.x = width * 0.21;
  playerB.maxbounds.y = height - 20;
  playerB.minbounds.y = height * 0.2;
